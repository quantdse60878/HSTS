...
public interface PatientRepo extends JpaRepository<Patient, Integer> {
    ...
    @Query(value = "select patient.* from Patient patient join MedicalRecord mr on patient.id = mr.patientId join Appointment a on mr.id = a.medicalRecordId where a.appointmentDateTime = :appointmentDateTime and a.status = 3 and mr.status in (1,1)", nativeQuery = true)
