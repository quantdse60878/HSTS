...
public interface NotifyRepo extends JpaRepository<Notify, Integer> {
    ...
    @Query("select n from Notify n where receiver.id = :receiverId and message = :message and type = :type and status = :status")
    public List<Notify> findUnreadNotifyByMessageContent(@Param("receiverId") final int receiverId,
            @Param("message") final String message, @Param("type") final byte type, @Param("status") final byte status);
}
